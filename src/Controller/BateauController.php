...
class BateauController extends AbstractController
{
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $port = $this->portRepository->find($data['id_port'] ?? 0);
        $proprietaire = $this->utilisateurRepository->find($data['id_utilisateur'] ?? 0);
        $typeBateau = $this->typeBateauRepository->find($data['id_type_bateau'] ?? 0);
